...
@ Modifications Made by Hayley Johnsey for CS 413-01 Lab 5
...
main:
	...
	mov r4, #0	@initialize blue button counter
	mov r5, #0	@initialize green button counter
	mov r6, #0	@initialize yellow button counter
	mov r7, #0	@initialize red button counter
	mov r3, #0	@initialize total button counter
...
@add the button presses to compare them to 10
    add r3, r3, r4
    add r3, r3, r5
    add r3, r3, r6
    add r3, r3, r7
...
@
    ...
    add    r4, r4, #1	@add one to the blue button pressed counter 
...
@
    ...
    mov    r10, r0
    add    r5, r5, #1	@add one to the green button pressed counter
    b      PedGREEN 
...
@
    ...
    mov    r11, r0
    add    r6, r6, #1	@add one to the yellow button pressed counter
    b      PedYELLOW 
...
@
    ...
    mov    r8, r0
    add    r7, r7, #1	@add one to the red button pressed counter
    b      PedRED
...
limitString:
   ldr r0, =limitStr	@load r0 with limitStr
   bl printf		@print limitStr

   ldr r0, =buttonStrB	@load r0 with buttonStrB (blue)
   bl printf
   mov r1, r4
   ldr r0, =numOutPattern
   bl printf

   ldr r0, =buttonStrG	@load r0 with buttonStrG (green)
   bl printf
   mov r1, r5
   ldr r0, =numOutPattern
   bl printf

   ldr r0, =buttonStrY	@load r0 with buttonStrY (yellow)
   bl printf
   mov r1, r6
   ldr r0, =numOutPattern
   bl printf

   ldr r0, =buttonStrR	@load r0 with buttonStrR (red)
   bl printf
   mov r1, r7
   ldr r0, =numOutPattern
   bl printf
...
@ Use the C library to print the goodbye string. 
    LDR  r0, =goodbye @ Put address of string in r0
    BL   printf       @ Make the call to printf
...
.balign 4
string1: .asciz "Welcome to the Button Pressing Lab! \n"
.balign 4
string1a: .asciz "This program will allow you to press buttons and will count the presses of each. \n" 
.balign 4
stringPressAny: .asciz "Press any of the buttons (Blue, Green, Yellow, or Red). \n"
.balign 4
string2: .asciz "Hardware error in GPIO see GTA. \n"
.balign 4
goodbye: .asciz "Program ending. . . \n"
...
.balign 4
limitStr: .asciz "The button pressing limit has been reached. \n"
.balign 4
buttonStrB: .asciz "The blue button was pressed the following times: \n"
.balign 4
buttonStrG: .asciz "The green button was pressed the following times: \n"
.balign 4
buttonStrY: .asciz "The yellow button was pressed the following times: \n"
.balign 4
buttonStrR: .asciz "The red button was pressed the following times: \n"
.balign 4
numOutPattern: .asciz "%d\n"
